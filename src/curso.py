...
# Método construtor
    ...
    def pre_requisitos(self, lista):
        if not isinstance(lista, list): # isinstance retorna True se for uma list
            raise TypeError("Os pré-requisitos devem ser uma lista.")
        for codigo in lista: # Verifica se cada elemento é um str
            if not isinstance(codigo, str):
                raise TypeError("Cada pré-requisito deve ser uma string.")
    #[:]serve  para salvar uma cópia e proteger a lista
        self.__pre_requisitos = lista[:]
